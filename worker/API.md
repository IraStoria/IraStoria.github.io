...
- 速率限制（KV key `rl:<route>:<ip>`，TTL 秒）：`submit` 5 次／10 分；`vote` 30 次／10 分；`wishes` 60 次／分。超過 → 429 `rate`。
...
→ 是：簽發通行證 `token = base64url(payload).base64url(HMAC-SHA256)`，payload `{ sub:login, exp: now+12h }`，302 到 `SITE_URL + '#wp=' + token`。
→ 否：302 到 `SITE_URL + '#wp=denied'`。
前端讀到 `#wp=` 立刻存 `sessionStorage.wp_token` 並清掉 fragment。

### 需 `Authorization: Bearer <token>` 的端點（無效／過期 → 401 `auth`）
